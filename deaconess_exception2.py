...
class deaconess_exception_report:
    ...
    def book_to_facs1(self):
        ...
        rslt_df.to_excel(writer, sheet_name="lead", index=False)
        return rslt_df
    ...
    def Amount_Validation(self, deaconess, book_to_facs, writer):
        try:
            # book_to_facs.to_excel(writer, sheet_name='Before', index=False)
            book_to_facs = self.book_to_facs_preprocess(book_to_facs)
            # book_to_facs.to_excel(writer,sheet_name='After',index=False)

            book_to_facs_clientid = book_to_facs[0]
            book_to_facs_clientid.to_excel(writer, sheet_name='After', index=False)
    ...
    def Exception_report(self,deaconess, book_to_facs, writer):
        try:
            ...
            book_to_facs_original.to_excel(writer, sheet_name="samp1", index=False)
            deaconess1 = self.billing_statement_preprocess(deaconess)
            deaconess_original = deaconess1[1]
            print(deaconess_original)
            ...
            dff1.to_excel(writer, sheet_name="samp", index=False)
...
if __name__ == "__main__":
    writer = pd.ExcelWriter('/home/ajay/Deconess_exception_report1.xlsx', engine='xlsxwriter')
    ...
    book_to_fac=obj.book_to_facs1()
    book_to_fac.to_excel("/home/ajay/book_to_facs_repo.xlsx", index=False)
    book_to_facs = pd.read_excel("/home/ajay/book_to_facs_repo.xlsx")
