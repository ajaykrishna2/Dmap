...
class deaconess_exception_report:
    def book_to_facs_preprocess(self,book_to_facs):
        try:
            book_to_facs.drop(book_to_facs.loc[book_to_facs['PRN'] == 0.0].index, inplace=True)
            book_to_facs = book_to_facs.loc[book_to_facs['Client Name'].isin(['Deaconess Gibson Hospital','Deaconess Health System',\
            'Deaconess Henderson Hospital','Deaconess Specialty Physicians','Deaconess Union County Hospital','THE HEART HOSPITAL'])] 
            ...
            book_to_facs['flag_id'] = book_to_facs["Acct#"].map(str) + book_to_facs["year"].map(str) +\
            book_to_facs['month'].map(str) + book_to_facs["day"].map(str) + book_to_facs['Pmt Type'].map(str)
    ...
    def billing_statement_preprocess(self,deaconess_hospital):
        try: 
            deaconess_hospital.drop(deaconess_hospital.loc[(deaconess_hospital['Due Agency'] == 0.0)&\
            (deaconess_hospital['Pd to Agency'] == 0.0)& (deaconess_hospital['Pd to You'] == 0.0)&\
            (deaconess_hospital['Due You'] == 0.0)].index, inplace=True)
            ...
            deaconess_hospital['flag_bill'] = deaconess_hospital["Clients Acct #"].map(str) +\
            deaconess_hospital["year"].map(str) + deaconess_hospital["month"].map(str) \
            + deaconess_hospital["day"].map(str) + deaconess_hospital['Statement_Total'].map(str)
            ...
            "Due Agency", "Due You"]]
            df = dataframe[['Client #', "Pmt Amt Applied", "Pd to Agency", "Pd to You", "Due Agency", "Due You"]]\
            .groupby(['Client #']).sum()
            df['Total'] = df['Pd to Agency'] + df['Pd to You']
            return [df.reset_index(), deaconess_hospital]
        except Exception as e:
    ...
    def Amount_Validation(self,deaconess_hospital,book_to_facs):
        try:
            ...
            client_id_level = pd.merge(book_to_facs_clientid,deaconess_clientid[['Client #', 'Pd to Agency', 'Pd to You',\
             'Total']],left_on="Client#", right_on="Client #", how='outer')
            ...
            save_file.send_statement(client_id_level12,'Amount Validation','Exec_Deaconess','output/Deaconess')
        except Exception as e:
    ...
    def Exception_report(self,deaconess_hospital,book_to_facs):
        try:
            ...
            dff1 = pd.merge(deaconess_original, book_to_facs_original[['Client#', "Acct#", 'flag_id', 'flag_book_to_FACS', 'PRN',\
            ...
            exception11 = exception[(exception['Client#'].isin(exception["Client #"])) \
            & (exception['Client #'].isin(exception["Client#"]))]
            exception11 = exception11[["Client #", "Clients Acct #", 'Pd to Agency', 'Pd to You',\
            "Due Agency", "Due You", "Statement_Total", "PRN", 'Int', 'Atty', 'Misc', 'CC', 'PJI', 'Total Pay', 'O/P Amt']]
            save_file.send_statement(exception11,'Balance Mismatch','Exec_Deaconess','output/Deaconess')
            ...
            statement_exception = statement_exception[["Client#", "Acct#", "PRN", 'Int', 'Atty', 'Misc', 'CC',\
            'PJI', 'Total Pay', 'O/P Amt']]
            save_file.send_statement(statement_exception,'Statement_Missing_Records','Exec_Deaconess','output/Deaconess')
            ...
            book_to_facs_exception = book_to_facs_exception[["Client #", "Clients Acct #", 'Pd to Agency',\
            'Pd to You', "Due Agency", "Due You", "Statement_Total"]]
            save_file.send_statement(book_to_facs_exception,'BookToFacs_Missing_Records','Exec_Deaconess','output/Deaconess')
            # Duplicate records in booktofacs and statement
            statement_dup = pd.concat(g for _, g in deaconess_original.groupby(["Clients Acct #", "Pmt Date", "Pmt Type",\
            'Pmt Amt Applied', 'Pd to Agency', 'Pd to You', 'Due Agency', 'Due You',"Client #"]) if len(g) > 1)
            statement_dup = statement_dup[["Clients Acct #", "Pmt Date", "Pmt Type", 'Pmt Amt Applied', 'Pd to Agency',\
            'Pd to You', 'Due Agency', 'Due You',"Client #", "First Name", "Last Name"]]
            save_file.send_statement(statement_dup,'Statement Miscellaneous','Exec_Deaconess','output/Deaconess')
            booktofacs_dup = pd.concat(g for _, g in book_to_facs_original.groupby(["FACS#", "Client#", "Acct#", 'Date',\
            'Total', 'Pmt Type', 'PRN', 'Int', 'Atty', 'Misc', 'CC', 'PJI','O/P Amt', "First Name", "Last Name"]) if len(g) > 1)
            booktofacs_dup = booktofacs_dup[["FACS#", "Client#", "Acct#", 'Date', 'Total', 'Pmt Type', 'PRN', 'Int', 'Atty',\
            'Misc', 'CC', 'PJI','O/P Amt', "Ref#", "First Name", "Last Name"]]
            save_file.send_statement(booktofacs_dup,'Book To Facs Miscellaneous','Exec_Deaconess','output/Deaconess')
        except Exception as e:
