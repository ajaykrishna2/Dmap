...
def Exception_report(writer):
    ...
    try:
        ...
        result = pd.merge(df[['Clients Acct #',  'Pmt Amt Applied', 'Pd to Agency', 'Pd to You', 'Due Agency','Due You','Client #','Total','flag_bill','flag_id']], book_to_facs[['PRN', 'O/P Amt','flag_book_to_FACS','flag_id']],on='flag_id',how='outer')
        result.reset_index(drop=True,inplace=True)
        df2=result[~result['flag_bill'].isin(result['flag_book_to_FACS'])]
        df2.reset_index(drop=True,inplace=False)
        df3=df2[['Clients Acct #', 'Client #', 'Pmt Amt Applied', 'Pd to Agency', 'Pd to You', 'Due Agency', 'Due You','Total' ,'PRN', 'O/P Amt']]
