...
class harrison_mva_reconcilation_report:
    def Harrison_MVA(self,df,inf):
        try:
            ...
            inf2= inf.rename(columns={'Issue Deteched':'Issue Detected','FACS Acct': 'FACS #', 'Client REF #': 'CLIENT ACCOUNT #',
                                 ...
                                 'Recon File BAL':'Recon file (Harrison) balance','Cancel DTE':'Cancel Date',
                                 'Last Pay DTE':'Last Payment Date','send dt/tm':'Send Dt/Tm'}, inplace=False)
            ...
            df23['Cancel Date'] = ''
            df26['Cancel Date'] = ''
            save_file.send_statement(df20, 'Closed in FACS','HA4_MVA_HOSP','Harrison_MVA')
            save_file.send_statement(df7, 'FACS not RECON','HA4_MVA_HOSP','Harrison_MVA')
            save_file.send_statement(df_r, 'RECON not FACS','HA4_MVA_HOSP','Harrison_MVA')
            save_file.send_statement(df26, 'Balance Mismatch','HA4_MVA_HOSP','Harrison_MVA')
            save_file.send_statement(df23, 'Balance Match','HA4_MVA_HOSP','Harrison_MVA')
            save_file.send_statement(dfw1, 'Multiples','HA4_MVA_HOSP','Harrison_MVA')
        
        except Exception as e:
            logging.exception("Error in processing harrison_mva recon file")
