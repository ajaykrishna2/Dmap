...
class Deaconess_reconcilation:
    ...
    logging.basicConfig(filename=local_output_directory+'Deaconess_reconcilation.log',filemode='a', level=logging.DEBUG,
                       format='%(asctime)s %(levelname)s %(name)s %(message)s')
    #Read required input
    obj = read()
    reconcilation_read = obj.collect_and_preprocess_deaconess_data()
    info_out_read=obj.info_out_read()
    reconcilation =deaconess_reconcilation_report()
    reconcilation.Deaconess(reconcilation_read,info_out_read)
