...
class Harrison:
    def Harrison(self,df,inf, writer):
        try:
            ...
            print(df[df['Issue Detected'] == "Account is in FACS, but not in the Harrison file"])
            ...
            print(inf.to_string())
            inf_med=inf[inf['Med1 BAL']==0]

            inf_med1=(inf_med[(inf_med['Cancel Code'].isna())])
            index_med1=inf_med1.index

            inf.drop(index_med1, inplace=True)
            inf_med2 = (inf_med[(inf_med['Cancel Code']==75)])
            index_med2 = inf_med2.index

            inf.drop(index_med2, inplace=True)
            inf_med3 = (inf_med[(inf_med['Cancel Code'] == 11)])
            index_med3 = inf_med3.index


            inf.drop(index_med3,inplace=True)


            inf1 = inf[inf.duplicated(['Client REF #'],keep=False)].sort_values('Client REF #')

            print(inf1.to_string())

            inf_cc=inf1[inf1.duplicated(['Cancel Code','Client REF #'],keep=False)].sort_values('Cancel Code')
            inf_cc['List DTE'] = pd.to_datetime(inf_cc['List DTE'])
            inf_cc['Date']=inf_cc.groupby(['Client REF #'])['List DTE'].transform('max')
            inf_cc['Date'] = pd.to_datetime(inf_cc['Date'])
            inf_cc['Match?'] = np.where(inf_cc['List DTE'] == inf_cc['Date'], 'True', 'False')
            inf_cc_index= inf_cc[inf_cc['Match?']=='False'].index


            inf_cc.drop(inf_cc_index,inplace=True)

            inf_cc.drop('Match?', axis=1, inplace=True)
            inf_cc.drop('Date', axis=1, inplace=True)
            inf.drop(inf1.index,inplace=True)
            inf1.drop(inf_cc_index,inplace=True)


            inf_null=inf1[(inf1['Cancel Code'].isna())]
            inf_75=inf1[inf1['Cancel Code']==75]
            inf_11=inf1[inf1['Cancel Code']==11]
            #
            inf1.drop(inf_null.index,inplace=True)
            inf1.drop(inf_75.index, inplace=True)
            inf1.drop(inf_11.index, inplace=True)
            inf_res = inf1



            preference_list = [4,71,75]
            # preference_list.insert(0,None)
            inf1=inf1[((inf1["Cancel Code"] ==4)|(inf1["Cancel Code"] ==71))]
            print(inf1.sort_values(["Client REF #"]).to_string())
            # inf_res.drop(inf1.index, inplace=True)
            inf1["Cancel Code"] = pd.Categorical(inf1["Cancel Code"], categories=preference_list, ordered=True)
            inf_1=inf1.sort_values([ "Cancel Code"]).drop_duplicates(subset=["Client REF #"])
            # print(inf_1.to_string())
            c1_index=inf1[~inf1.index.isin(inf_1.index)]
            inf_res.drop(c1_index.index, inplace=True)
            inf_res.drop(inf_1.index,inplace=True)
            inf1=pd.concat([inf_1,inf_null,inf_11])
            # print(inf1.to_string())
            inf1 = inf1.sort_values(["Client REF #"])
            inf1['Match?'] = np.where((inf1['Cancel Code'] == 4)|(inf1['Cancel Code'] == 71), 'True', 'False')
            inf_cli=inf1[(inf1['Match?']=='True')]

            # print(inf_cli.index)
            inf_c2=inf_cli["Client REF #"]
            inf_c3=inf_cli["Client REF #"].tolist()
            # print(inf_c3)
            inf1.drop(inf1[inf1['Client REF #'].isin(inf_c3)].index,inplace=True)
            inf_c2=inf_c2.reset_index()
            # print(inf1[inf_c2["Client REF #"]])
            # print(inf_cli["Client REF #"])
            # inf1.loc[(inf1['Cancel Code'] == 4) | (df['Cancel Code'] == 71), 'Match'] = 'True'
            # inf1.loc[(inf1['Cancel Code'] != 4) | (df['Cancel Code'] != 71), 'Match'] = 'False'


            # print(inf_res.to_string())

            # inf1=pd.concat([inf1,inf_75])
            # preference_list = [ 4,71,75]
            # inf1["Cancel Code"] = pd.Categorical(inf1["Cancel Code"], categories=preference_list, ordered=True)
            # inf1 = inf1.sort_values(["Cancel Code"]).drop_duplicates(subset=["Client REF #"])

            # inf1=pd.concat([inf1,inf_null,inf_11,inf])
            #
            # inf1=pd.concat([inf1,inf_1])

            print(inf1.to_string())

            inf2=inf1.sort_values(by="List DTE").drop_duplicates(subset=["Client REF #"], keep="last")
            print(inf2.to_string())
            inf2=pd.concat([inf,inf2,inf_cli])
            print(len(inf2))
            ...
            print(inf3)
            # print(pd.merge(inf2,df8, on='CLIENT ACCOUNT #',how='left').to_string())
            ...
            # print(len(inf3[inf3['Last Payment Date'].isna()]))

            # print(inf3[inf3.duplicated(['CLIENT ACCOUNT #'], keep=False)].sort_values('CLIENT ACCOUNT #'))
            ...
            print(df7[df7.duplicated(['CLIENT ACCOUNT #'], keep=False)].sort_values('CLIENT ACCOUNT #').to_string())
            ...
            df_r.to_excel(writer, index=False, sheet_name='RECON not FACS')
