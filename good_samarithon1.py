...
class Good_sam:
    def Good_sam(self,df, writer):
        try:
            df133=df[df['Issue Detected'] == "EPIC has this account in Early Out, but the account is in the 3000 phase."]
            df.drop(df133.index,inplace=True)

            df_rec=df[df['Client ID'].str.contains("REC", na=False, regex=True)]
            df.drop(df_rec.index, inplace=True)
            ...
            df_rec.to_excel(writer, index=False, sheet_name='REC accts')
            # dfl.to_excel(writer, index=False, sheet_name='legal account')
            df133.to_excel(writer, index=False, sheet_name='Phase Mismatch')
...
if __name__ == "__main__":
    writer = pd.ExcelWriter('/home/ajay/GSH_Recon_080321_50099.xlsx', engine='xlsxwriter',options={'strings_to_numbers':True} )
