...
class parkview_reconcilation_report:
    def Parkview(self,df):
        try:
            ...
            save_file.send_statement(df16, 'IFU Closed in FACS','Rec_Parkview','Parkview')
            save_file.send_statement(df17, 'WCP Closed in FACS','Rec_Parkview','Parkview')
            save_file.send_statement(df18, 'IFU FACS not RECON','Rec_Parkview','Parkview')
            save_file.send_statement(df19, 'WCP FACS not RECON','Rec_Parkview','Parkview')
            save_file.send_statement(df7, 'RECON not FACS','Rec_Parkview','Parkview')
            save_file.send_statement(df22, 'IFU Balance Mismatch','Rec_Parkview','Parkview')
            save_file.send_statement(df23, 'WCP Balance Mismatch','Rec_Parkview','Parkview')
            save_file.send_statement(df24, 'IFU Balance Match','Rec_Parkview','Parkview')
            save_file.send_statement(df25, 'WCP Balance Match','Rec_Parkview','Parkview')
        
        except Exception as e:
            logging.exception("Error in processing parkview recon file")
