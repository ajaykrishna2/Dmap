...
def Riverview(df, writer):
    try:
        ...
        # num = Sheet2._get_numeric_data()
        # num[num < 0] = '(' + Sheet2['Adjustments'].astype(str) + ')'
        # num = str(Sheet2.translate(None, ",()"))
        ...
        # table6=table3.style.apply(lambda x: ['background-color: yellow' if s > 0 else '' for s in x])
        # table6 = table3.style.apply(highlight_col, axis=None)
        # display(table6)
        table6 = table3.style.apply(color, axis=None)
        ...
        table5.to_excel(writer, index=True, sheet_name='CBS Work Comp_Accident')
        data_details1.to_excel(writer, index=False, sheet_name='Details')
        i1=pd.MultiIndex.from_frame(table6)
        i2=index.get_level_values(0)
        print(i2)
