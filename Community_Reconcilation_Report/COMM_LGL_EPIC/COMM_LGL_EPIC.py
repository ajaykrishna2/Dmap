from Utility.comm_epic_lgl_reconcilation import *
...
class Comm_epic_lgl_reconcilation:
    print("starting data ::::::::::: ",datetime.datetime.now())
    # creation of log file
    logging.basicConfig(filename=local_output_directory+'Comm_epic_lgl_reconcilation.log',filemode='a', level=logging.DEBUG,
                       format='%(asctime)s %(levelname)s %(name)s %(message)s')
    #Read required input
    obj = read()
    reconcilation_read = obj.collect_and_preprocess_comm_epic_lgl_data()
    reconcilation =comm_epic_lgl_reconcilation_report()
    reconcilation.Comm_epic_lgl(reconcilation_read)
