...
class comm_epic_lgl_reconcilation_report:
    def Comm_epic_lgl(self,df):
        try:
            ...
            ad=df4[df4['Cancel Code']==7.0]
            df4.drop(ad.index,inplace=True)
            index_name2 = df4[((df4['Cancel Code']==2.0)|(df4['Cancel Code']==4.0)) & (df4['year3'] >= df4['year1']) & (df4['month3'] >= df4['month1']) & (df4['day3'] > df4['day1'])].index
            df4.drop(index_name1, inplace=True)
            ...
            save_file.send_statement(df4, 'Closed in FACS','COMM_LGL_EPIC','Rec_Community')
            save_file.send_statement(df6, 'FACS not RECON','COMM_LGL_EPIC','Rec_Community')
            save_file.send_statement(df5, 'RECON not FACS','COMM_LGL_EPIC','Rec_Community')
            save_file.send_statement(df10, 'Balance Mismatch','COMM_LGL_EPIC','Rec_Community')
            save_file.send_statement(df13, 'Balance Match','COMM_LGL_EPIC','Rec_Community')
            save_file.send_statement(ad, 'Adjustments','COMM_LGL_EPIC','Rec_Community')
        
        except Exception as e:
            logging.exception("Error in processing community recon file")
