...
class read:
    ...
    def collect_and_preprocess_comm_epic_lgl_data(self):
        try:
            ...
            for i in filtered_list:
              if 'Rec_Community' in i and str(read.year) in i and read.month_name in i and 'COMM_LGL_EPIC' in i:
                ...
                comm_statement = pd.read_table(io.BytesIO(data),sep="\t",skiprows=1,index_col=False, encoding="ISO-8859-1")
                comm_statement.columns=['Issue Detected','FACS #','EPIC #','Client ID','First Name','Last Name','Disposition','Phase','List Date','Med-1 Balance','Amount Cancelled','Recon file (EPIC) balance','Last Payment Date','Cancel Code','Cancel Date','Cancel Description','Legal Flag']
            return comm_statement

   
        except Exception as e:
            logging.exception("error in reading community_reconcilation file")
