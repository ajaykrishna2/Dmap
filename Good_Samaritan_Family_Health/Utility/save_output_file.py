...
class save_statement_to_output_folder:
    ...
    def save_file_to_s3(cls):
        ...
        path_s3 ='BA_output_folder/Good_Samaritan_Family/'+ str(year)+"/"+month_name+"/" +zipped_file.split("/")[::-1][0]
        my_bucket.upload_file(zipped_file, path_s3)
        for j in xls_list:
            if (".zip" not in j):
                y = j.split("/")[::-1][0];
                z = y.replace(".xlsx", "")
                excle_s3 = "Exception_Report_Input" + "/" + "Exec_Good_Samaritan_Family_Health" + "/" + str(year) + "/" +\
                month_name + "/" + y
