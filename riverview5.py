...
def Riverview(df, writer):
    try:
        ...
        table3.set_index(index1)
        table3['NAME']=table3['NAME'].shift(1)
        table3.index.delete(1)

        # del table6['NAME']
        # del table6['CLIENT']
        # table6=table3
        table6 = table3.style.hide_index()
        # table6=table3
        ...
        table6.to_excel(writer, index=False, startcol=4,sheet_name='MED-1 Collections')
        ...
        worksheet1.conditional_format('M2:M1000', {'type': 'cell','criteria': '<','value': 0,'format': header_format3})
        worksheet2.conditional_format('I2:I1000', {'type': 'cell','criteria': '<','value': 0, 'format': header_format3})
        worksheet3.conditional_format('I2:I1000', {'type': 'cell','criteria': '<','value': 0,'format': header_format3})
        format2 = workbook.add_format({'num_format': '(00000000)'})
        worksheet3.set_column('I:I', 9, format2)
        ...
        # df21=table6.data
        # print(df21['Adjustments'])
        # Sheet2["Adjustments"] = df["Adjustments"].astype(str).apply(lambda x: f"({x})" if float(x) < 0.0 else x)
