...
def Riverview(df, writer):
    try:
        def color_negative_red(val):
            ...
            color = 'red' if val['Adjustments'] < 0 else 'black'
        ...
        def highlight_col(x):
            ...
            #     is_neg = [False for _ in range(x.shape[0])]
            # else:
            #     is_neg = x < 0
            ...
            # return df1,'color: %s' % color
            # color = 'red' if x.startswith('(') else 'black'
            # return df1
            # if x.dtype == np.object:
            #     is_neg = [False for _ in range(x.shape[0])]
            # else:
            #     is_neg = x < 0
            # return df1,['color: red;' if cell else 'color:black'
            #         for cell in is_neg]
        ...
        # Sheet2["Adjustments"] = Sheet2["Adjustments"].apply(pd.to_numeric)
        print(Sheet2["Adjustments"])
        # Sheet2["Adjustments"]=Sheet2["Adjustments"].astype(str).apply(lambda x: f"({x})" if float(x)<0.0 else x)
        # Sheet2["Adjustments"] =pd.to_numeric(Sheet2["Adjustments"], errors='coerce').astype('float64')
        # Sheet2["Adjustments"] = Sheet2.apply(lambda x: "{:,.0f}".format(x["Adjustments"]), axis=1)
        # Sheet2["Adjustments"]=Sheet2["Adjustments"].astype(int)
        ...
        # y = 'color:red'
        # table3.loc[table3['Adjustments'] < 0 ] = y
        ...
        # table3['Adjustments'] = table3['Adjustments'].apply(lambda x: f'({abs(x)})' if x < 0 else x)
        # table3['Adjustments'] = pd.to_numeric(table3['Adjustments'], errors='coerce').fillna(0).astype(np.int64)
        # table3['Adjustments']=table3.style.applymap(lambda x: "background-color: red" if x['Adjustments']<0 else "background-color: black")
        # table3['Adjustments'] = table3['Adjustments'].apply(lambda x: color_negative_red(x))
        #table3['Adjustments'] =table3['Adjustments'].apply(highlight_max)
        # table3['Adjustments'] = table3['Adjustments'].apply(lambda x: color = 'red' if x < 0 else 'black' return 'color: %s' % color)
        ...
        # Sheet4["Adjustments"]=Sheet4["Adjustments"].astype(float)
        Sheet4["Adjustments"] = Sheet4["Adjustments"].apply(pd.to_numeric)
        ...
        # Sheet6["Adjustments"]=Sheet6["Adjustments"].astype(float)
        Sheet6["Adjustments"] = Sheet6["Adjustments"].apply(pd.to_numeric)
        ...
        table6.to_excel(writer, index=True, sheet_name='MED-1 Collections')
        ...
        header_format2 = workbook.add_format({
            ...
            'fg_color': '#FAD2B9',
            'border': 1})
        header_format3 = workbook.add_format({'color':'#FF0000'})
        header_format4 = workbook.add_format({'fg_color': '#6B6565'})
        worksheet1.conditional_format('I2:I1000', {'type': 'cell',
                                              'criteria': '<',
                                              'value': 0,
                                              'format': header_format3})
        worksheet2.conditional_format('I2:I1000', {'type': 'cell',
                                                   'criteria': '<',
                                                   'value': 0,
                                                   'format': header_format3})
        worksheet3.conditional_format('I2:I1000', {'type': 'cell',
                                                   'criteria': '<',
                                                   'value': 0,
                                                   'format': header_format3})
        worksheet3.conditional_format('IE2:O2', {'type': 'row',
                                                   'format': header_format4})
        for col_num, value in enumerate(table6.columns.values):
            worksheet1.write(0, col_num + 4, value, header_format)
            if ((value=='Unadjusted %')|(value=='adjusted %')):
                worksheet1.write(0, col_num+4 ,value,header_format2)
        for col_num, value in enumerate(table7.columns.values):
            worksheet2.write(0, col_num + 4, value, header_format)
            if ((value=='Unadjusted %')|(value=='adjusted %')):
                worksheet2.write(0, col_num+4 ,value,header_format2)
        for col_num, value in enumerate(table8.columns.values):
            worksheet3.write(0, col_num +4, value, header_format)
            if ((value=='Unadjusted %')|(value=='adjusted %')):
                worksheet3.write(0, col_num+4 ,value,header_format2)
        for col_num, value in enumerate(index1):
        ...
        for col_num, value in enumerate(index1):
            ...
            print(table6.data)
        # for col_num, value in enumerate(table6.data):
        #     print(value)
        #     worksheet3.write(1, col_num+4 , value, header_format3)
        # for row, row_data in enumerate(table6.data):
        #     worksheet1.write_row(row + 0, 1, row_data, header_format)
