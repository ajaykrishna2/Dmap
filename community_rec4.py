...
class Communityhospital:
    def communityhospital(self,df, writer):
        try:
            ...
            # num_cols = df.shape[1]
            # print(num_cols)
            # df.columns = ['Issue Detected', 'FACS #', 'EPIC #', 'Client ID', 'First Name', 'Last Name', 'Disposition',
            ...
            # df = df.rename(columns={'Legal Flag': ''})
...
if __name__ == "__main__":
    writer = pd.ExcelWriter('/home/ajay/COMMUNITY_EPIC_LGL_RECON_041221_61571.xlsx', engine='xlsxwriter',options={'strings_to_numbers':True} )
    path_name='/home/ajay/Downloads/COMMUNITY_EPIC_LGL_RECON_041221_61571.TXT'

    df = pd.read_csv(path_name,sep='\t',names=['Issue Detected','FACS #','EPIC #','Client ID','First Name','Last Name','Disposition','Phase','List Date','Med-1 Balance','Amount Cancelled','Recon file (EPIC) balance','Last Payment Date','Cancel Code','Cancel Date','Cancel Description','Legal Flag','samp'],skiprows=1)
    # df= pd.read_csv(path_name,sep='\\t',lineterminator='\\r',header='infer', engine='python')
    print(df)
    # df.columns=['Issue Detected','FACS #','EPIC #','Client ID','First Name','Last Name','Disposition','Phase','List Date','Med-1 Balance','Amount Cancelled','Recon file (EPIC) balance','Last Payment Date','Cancel Code','Cancel Date','Cancel Description','Legal Flag']
    df_facs1 = df
