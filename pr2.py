...
def Riverview(df, writer):
    try:
        def highlight_col1(x):
            ...
            #      'NOV': 11, 'DEC': 12}
            # x['monthnumber'] = x.Month.map(d)
            # x['rank'] = x.groupby(['NAME', 'CLIENT', 'Year'])['monthnumber'].rank(ascending=True);
            # table3_2 = x.index[x['rank'] != 1.0]
            # j = list(table3_2)
            i = x.index[(x.duplicated(['CLIENT']))]
            j = x.index[(x.duplicated(['CLIENT','Year']))]
            df2.iloc[j, 2] = w
            df2.iloc[i, 0] = w
            df2.iloc[:,1]=w
            # x[["NAME", "CLIENT", "Year", "Month", "# of Accts Listed", "Amt Listed", "Recovered", "Unadjusted %",
            #      "Adjustments", "RECALLED", "Returned", "Adjusted %", "Inventory Remaining", "FEES", 'rank',
            #      "AVG AGE AT LIST"]]
            # df2.iloc[:, 15] = w
            # df2.iloc[:, 16] = w
            # x.drop(['rank'],axis=1)
        ...
        table6 = tab1[["NAME", "CLIENT", "Year", "Month", "# of Accts Listed", "Amt Listed", "Recovered", "Unadjusted %","Adjustments","RECALLED", "Returned", "Adjusted %", "Inventory Remaining", "FEES","AVG AGE AT LIST"]].style.apply(highlight_col1, axis=None).set_properties(**{'text-align': 'right'}).set_table_styles(styles)
        ...
        # h=["NAME", "CLIENT", "Year", "Month", "# of Accts Listed", "Amt Listed", "Recovered", "Unadjusted %","Adjustments","RECALLED", "Returned", "Adjusted %", "Inventory Remaining", "FEES", 'rank',"AVG AGE AT LIST"]
        ...
        worksheet1.conditional_format("B2:B1000",
                                      {'type': 'formula', 'criteria': '(COUNTIF($B$2:B2,B2)=1)', 'format': format1})
        worksheet2.conditional_format("B2:B1000",
                                      {'type': 'formula', 'criteria': '(COUNTIF($B$2:B2,B2)=1)', 'format': format1})
        worksheet3.conditional_format("B2:B1000",
                                      {'type': 'formula', 'criteria': '(COUNTIF($B$2:B2,B2)=1)', 'format': format1})
